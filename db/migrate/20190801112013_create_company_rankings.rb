    ...
    execute <<-SQL
      create materialized view if not exists company_rankings as (
        with recursive r(rankable_type, rankable_id, company_id, year, value, count, rank) as (
          select 'Outcome', o.id, c.id, y, value,
          ...
          select 'Group', group_id, company_id, year, avg_rank, count,
          ...
          from (
            select distinct group_id, company_id, year,
              avg(r.rank)    over(partition by group_id, company_id, year) as avg_rank,
              count(r.value) over(partition by group_id, company_id, year) as count
